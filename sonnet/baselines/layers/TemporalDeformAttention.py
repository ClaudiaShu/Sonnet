...
from sonnet.baselines.layers.MLP import MLP
...
class DeformAtten1D(nn.Module):
    ...
    def __init__(
        ...
        self.proj_q = nn.Conv1d(
            self.d_model, self.d_model, kernel_size=1, stride=1, padding=0
        )
    ...
    def forward(self, x, mask=None):
        ...
        if self.rpb:
            ...
            v = (v + self.relative_position_bias_table).reshape(
                B * self.n_heads, self.n_head_channels, L
            )
        else:
        ...
        attention = torch.softmax(
            scaled_dot_prod, dim=-1
        )  # softmax: attention[0,0,:].sum() = 1
...
class DeformAtten2D(nn.Module):
    ...
    def __init__(
        ...
        self.proj_offset = nn.Sequential(
            ...
            nn.Conv2d(
                self.n_group_channels, 2, kernel_size=1, stride=1, padding=0, bias=False
            ),
        )
    ...
    def forward(self, x, mask=None):
        ...
        if not self.no_off:
            x_sampled = rearrange(
                x_sampled, "(b g) c h w -> b (g c) h w", g=self.n_groups
            )
...
class CrossDeformAttn(nn.Module):
    def __init__(
        ...
        # Deform attention
        self.deform_attn = DeformAtten1D(
            self.subseq_len,
            d_model,
            n_heads,
            dropout,
        ...
        self.mlps1d = nn.ModuleList(
            [
                MLP([d_model, d_model], final_relu=True, drop_out=0.0)
                for _ in range(len(self.attn_layers1d))
            ]
        )
        self.drop_path1d = nn.ModuleList(
            [
                DropPath(droprate) if droprate > 0.0 else nn.Identity()
                for _ in range(len(self.attn_layers1d))
            ]
        )
        #######################################
        ...
        self.mlps2d = nn.ModuleList(
            [
                MLP([d_model, d_model], final_relu=True, drop_out=0.0)
                for _ in range(len(self.attn_layers2d))
            ]
        )
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        x_1d = rearrange(
            x_padded, "b (seg_num ts_d) d_model -> (b ts_d) seg_num d_model", ts_d=n_day
        )
        # attn on 1D
        for d, attn_layer in enumerate(self.attn_layers1d):
            ...
            x_1d = self.drop_path1d[d](x_1d) + x0
        x_1d = rearrange(
            x_1d, "(b ts_d) seg_num d_model -> b (seg_num ts_d) d_model", ts_d=n_day
        )[:, :L, :]
        ...
        for d, attn_layer in enumerate(self.attn_layers2d):
            ...
            x0 = x_2d
            x_2d = self.mlps2d[d](self.layer_norm(x_2d.permute(0, 1, 3, 2))).permute(
                0, 1, 3, 2
            )
            x_2d = self.drop_path2d[d](x_2d) + x0
