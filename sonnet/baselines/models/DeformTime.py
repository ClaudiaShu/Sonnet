...
from sonnet.baselines.layers.TemporalDeformAttention import Encoder, CrossDeformAttn
from sonnet.baselines.layers.Embed import (
...
class Model(BaseModel):
    def __init__(self, configs, **kwargs) -> None:
        ...
        # Embedding
        if configs.enc_in == 1:
            self.enc_value_embedding = Deform_Temporal_Embedding(
                self.f_dim, self.d_model, freq="d"
            )
        else:
            self.s_group = configs.n_heads
            ...
            self.enc_value_embedding = Local_Temporal_Embedding(
                self.pad_in_len // self.s_group,
                self.d_model,
                self.pad_in_len - configs.enc_in,
                self.s_group,
            )
        ...
        dpr = [
            x.item()
            for x in torch.linspace(drop_path_rate, drop_path_rate, self.e_layers)
        ]
        ...
        self.ff = (
            nn.Linear(self.f_dim, self.c_out) if self.c_out == 1 else nn.Identity()
        )
    ...
    def forecast(self, x_enc):
        ...
        # Decoder
        h0 = (
            torch.zeros(self.d_layers, x_enc.size(0), self.d_model)
            .requires_grad_()
            .to(x_enc.device)
        )
