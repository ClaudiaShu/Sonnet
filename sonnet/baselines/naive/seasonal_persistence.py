...
class Model(BasePersistenceModel):
    ...
    def __init__(self, configs, **kwargs):
        ...
        self.seasonality = configs.seasonality
        self.target_idx = -1
...
class SeasonalPersistenceModel(L.LightningModule):
    """
    A seasonal persistence model that uses historical seasonal patterns to make predictions.
    The model assumes that the pattern from the previous season will repeat.
    """
...
def run_seasonal_persistence(
    ...
    # Initialize model
    model = SeasonalPersistenceModel(
        seq_length=seq_length, pred_length=pred_length, seasonality=seasonality
    )
...
if __name__ == "__main__":
    ...
    # Run model
    results = run_seasonal_persistence(
